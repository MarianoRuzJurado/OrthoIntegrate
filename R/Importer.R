...
Importer <- function(pathways,ids, TenX=TRUE, performNormalisation=TRUE, performScaling = FALSE,performVariableGeneDetection=TRUE, FilterCells=TRUE, FilterByAbsoluteValues=FALSE,...) {
  ...
  for (i in 1:length(pathways)) {
    ...
    #write QC to file
    p1<-Seurat::VlnPlot(object = seuratObject, features = c("nFeature_RNA"), ncol = 1, pt.size = 0) + ggplot2::theme(legend.position = "None", axis.title.x = ggplot2::element_blank(), axis.text.x = ggplot2::element_blank())
    ...
    ggplot2::ggsave(filename = paste0(pathway,"QC_preFiltered.svg"),device = "svg", width = 10,height = 10)
