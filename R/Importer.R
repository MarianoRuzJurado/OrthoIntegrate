...
Importer <- function(pathway,id, TenX=TRUE, performNormalisation=TRUE, performScaling = FALSE,performVariableGeneDetection=TRUE, FilterCells=TRUE, FilterByAbsoluteValues=FALSE,...) {
  ...
  #write QC to file
  p1<-Seurat::VlnPlot(object = seuratObject, features = c("nFeature_RNA"), ncol = 1, pt.size = 0) + ggplot2::theme(legend.position = "None", axis.title.x = element_blank(), axis.text.x = element_blank())
  p2<-Seurat::VlnPlot(object = seuratObject, features = c("nCount_RNA"), ncol = 1, pt.size = 0) + ggplot2::theme(legend.position = "None", axis.title.x = element_blank(), axis.text.x = element_blank())
  p3<-Seurat::VlnPlot(object = seuratObject, features = c("percent.mito"), ncol = 1, pt.size = 0) + ggplot2::theme(legend.position = "None", axis.title.x = element_blank(), axis.text.x = element_blank())
